...
void skimonefile(TString inputfile, TString outputdir, TString skim) 
{
  ...
  if(skim=="rpvfit") {
		...
		//if(inputfile.Contains("JetHTRun")) skimcut = "ht>1200 && njets>=3 && mj12>500 && nbm<2 && nleps==2";
...
//int main()
{
  ...
  else 
  {
    ...
    outputdir = inputdir;
    outputdir.ReplaceAll("processed_1013_tt", Form("skim_%s_1013_tt", skim.Data()));
  ...
  //vector<TString> files = globVector(Form("%s/*JetHTRun2018B*.root", inputdir.Data())); 
