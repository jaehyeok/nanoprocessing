...
int main(){
	...
	TFile *f_bef = new TFile("btagEfficiency_signal_m1600.root","READ"); 
...
// */
 	...
 	for(int i=0; i<1000000 ; i++){
		...
		for(int j=0; j<rand_njets; j++){
			...
			Int_t rand_flav = gRandom->Integer(5);
			if(rand_flav<4) rand_flav=0;

			Int_t rand_rand = gRandom->Integer(99);
			...
			w_bctag_up = w_bctag_up*getBtagWeight(f_bef, calibreader, rand_pt, rand_eta, rand_flav, csv, 0.6321, "up_hf");
			w_bctag_down = w_bctag_down*getBtagWeight(f_bef, calibreader, rand_pt, rand_eta, rand_flav, csv, 0.6321, "down_hf");
			w_lftag_up = w_bctag_up*getBtagWeight(f_bef, calibreader, rand_pt, rand_eta, rand_flav, csv, 0.6321, "up_lf");
			w_lftag_down = w_bctag_down*getBtagWeight(f_bef, calibreader, rand_pt, rand_eta, rand_flav, csv, 0.6321, "down_lf");
